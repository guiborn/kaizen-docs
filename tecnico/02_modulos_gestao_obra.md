...
| — | [**Medição e Análise KAI**](03_modulo_medicao_kai_analysis.md) | Tabela de medição, análise de problemas, análise de caminho crítico, PPCC, sugestões automáticas |
| 3 | [**Controle de Produção**](#3-controle-de-produção) | Painel de produção, equipes e histograma, planos de ação |
...
> **Documentação detalhada:** [02b\_planejamento\_fisico\_financeiro.md](02b_planejamento_fisico_financeiro) — inclui arquitetura, fluxo de dados, controladores, algoritmos de cálculo e considerações de performance de cada sub-módulo.
...
> Requer cronograma ativo na obra. Itens 2.1–2.8 exigem privilégio **`baseline_view`**; item 2.9 exige **`physical`**.

| Sub-módulo | Descrição resumida |
|-----------|-------------------|
| **2.1 Atividades** | Tabela SfDataGrid com edição inline de datas/custos, filtros rápidos, pilha de desfazer (15 passos) e exportação Excel/PDF |
| **2.2 Macaquinho** | Visão matricial lote×serviço com tiles coloridos por status, heatmaps, menu de contexto para restrições/planos e exportação |
| **2.3 Gantt** | Barras em canvas (`CustomPainter`), reprogramação por drag & drop, pilha de desfazer, cálculo de caminho crítico local |
| **2.4 Curva S Acumulada** | Gráfico de linhas Base/Previsto/Realizado com trackball, zoom/pan e exportação CSV |
| **2.5 Gráfico Mensal** | Barras de delta mensal com drill-down interativo por serviço |
| **2.6 Análise de Serviços** | Grade mensal por serviço/atividade com filtros de período e sincronização de meses com o CFF |
| **2.7 CFF** | Cronograma Físico-Financeiro: WBS × meses em R$/% com modos Base/Previsto/Realizado e cache no Firestore |
| **2.8 Budget Weights** | Vinculação de itens WBS a atividades com percentuais de peso para cálculo do CFF |
| **2.9 Gestão de Cronogramas** | Ciclo de vida: criação, baseline, importação (Prevision / Excel / JSON) e histórico de revisões |
...
> **Documentação detalhada:** Ver [Módulo de Medição e Análise KAI](03_modulo_medicao_kai_analysis.md) para referência completa sobre tabela de medição, análise de problemas, análise de caminho crítico, PPCC e sugestões automáticas via IA.

---

### 3.1 Medições Físicas

O módulo de **Medições Físicas** é o núcleo do controle de produtividade da obra. Implementado com `SfDataGrid` (Syncfusion) para suporte a edição colaborativa offline-first, detecção automática de problemas via IA e integração com análise de caminho crítico (KAI Analysis Tab).
